...
from saadt.model import ArtifactBadge, Paper, PaperTitle, UsenixArtifactBadge
...
USENIX_BASE_URL = "https://www.usenix.org/conference/usenixsecurity{year}/"

log = logging.getLogger(__name__)


class UsenixScraperWorker(ScraperWorker[tuple[PaperTitle, str]]):
    ...
    def _parse_artifact_badge_link(self, url: str) -> ArtifactBadge | None:
        if "available" in url:
            return UsenixArtifactBadge.AVAILABLE
        if "functional" in url:
            return UsenixArtifactBadge.FUNCTIONAL
        if "reproduced" in url:
            return UsenixArtifactBadge.REPRODUCED
        return None
...
class UsenixOldScraperWorker(UsenixScraperWorker):
    def __init__(
        self,
        edition: str,
        **kwargs: Unpack[WorkerParams[tuple[PaperTitle, str], Paper]],
        ...
        self._paper_href_re = re.compile(
            rf"files/(?:conference/usenixsecurity{self.edition}/)?sec{self.edition}(?!_slides_|_web_flyer)(?:[-_][a-z]+[0-9]*)+(?:_[0-9])?\.pdf",
            re.IGNORECASE,
        )

    def _parse_artifact_badge_link(self, url: str) -> ArtifactBadge | None:
        if "passed" in url:
            return UsenixArtifactBadge.PASSED
        return None
...
class UsenixPreScraperWorker(UsenixScraperWorker):
    def __init__(
        self,
        edition: str,
        **kwargs: Unpack[WorkerParams[tuple[PaperTitle, str], Paper]],
        ...
        super().__init__(edition, **kwargs)
        self._paper_href_re = re.compile(
            r"files/sec\d{2}(?:summer|fall|winter|spring)(?:[-_][a-z0-9]+)+(?:_[0-9])?\.pdf", re.IGNORECASE
        )

    def _get_paper_link(self, soup: BeautifulSoup, name: str = "") -> str | None:
        node: bs4.Tag = soup.find("a", href=self._paper_href_re)  # type: ignore[assignment]
        if node is None:
            return None
        return node.get("href")  # type: ignore[return-value]


class UsenixScraper(ThreadedScraper[tuple[PaperTitle, str]]):
    @override
    def _get_papers(self) -> list[tuple[PaperTitle, str]]:
        ...
        try:
            ...
            raise RuntimeError("Error fetching USENIX website") from exc
    ...
    def _get_url(self, url: str) -> str:
        return urljoin(USENIX_BASE_URL.format(year=self.edition), url)
    ...
    def _worker(
        ...
        **kwargs: Unpack[mputils.WorkerParams[tuple[PaperTitle, str], Paper]],
        ...
        if int(args[0]) >= 22:
            return UsenixScraperWorker(*args, **kwargs)
        else:
            return UsenixOldScraperWorker(*args, **kwargs)


class UsenixPreScraper(UsenixScraper):
    @override
    def _get_papers(self) -> list[tuple[PaperTitle, str]]:
        log.info("Fetching conference papers")
        ...
        for page in ["summer-accepted-papers", "fall-accepted-papers"]:
            try:
                r = self.session.get(self._get_url(page))
                r.raise_for_status()
            except requests.HTTPError as exc:
                raise RuntimeError("Error fetching USENIX website") from exc
            ...
            soup = BeautifulSoup(r.content, "lxml")
            # find all papers
            nodes = self._get_tags(soup.find_all("article", class_="node-paper"))
    ...
    def _worker(
        ...
        **kwargs: Unpack[mputils.WorkerParams[tuple[PaperTitle, str], Paper]],
        ...
        return UsenixPreScraperWorker(*args, **kwargs)
